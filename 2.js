(() => {
	...
	const lines =
		...
		.map(line => line.split(' '))
		.map(([range, letter, password]) => ({range, letter, password}))

	const valid = lines
		.filter(line => {
			const matched = Array.from(line.password).filter(letter => letter == line.letter[0])
			const limits = line.range.split('-')
			return matched.length >= limits[0] && matched.length <= limits[1]
		})
	console.log({valid})
	console.log("Result: ", valid.length)
})()
